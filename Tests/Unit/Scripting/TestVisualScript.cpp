...
// ===========================================================================
// Script-API node codegen (Api_* nodes generated by reflection)
// ===========================================================================

ENJIN_TEST(ApiCodegen, GeneratesNodesAndExecutesThem) {
    // A real bound engine: registration reflects it, execution calls into it.
    Scripting::ScriptEngine se;
    ENJIN_ASSERT_TRUE(se.Initialize());
    Scripting::RegisterAllBindings(se.GetASEngine());

    auto& reg = NodeRegistry::Instance();
    usize before = reg.GetAllNodes().size();
    u32 made = RegisterScriptApiNodes(se.GetASEngine());
    // Idempotent guard means a previous test binary run in-process would
    // return 0; in this standalone test it must generate a large batch.
    ENJIN_ASSERT_TRUE(made > 500);
    ENJIN_EXPECT_EQ(reg.GetAllNodes().size(), before + made);

    // Hand-made nodes were NOT shadowed: the hand typeId still resolves and
    // no Api_ duplicate exists for it.
    ENJIN_ASSERT_TRUE(reg.FindNode(NodeTypes::UISetText) != nullptr);
    ENJIN_EXPECT_TRUE(reg.FindNode("Api_UI_SetText") == nullptr);

    // A generated PURE getter end-to-end: Time_GetScale defaults to 1.
    const NodeDefinition* getScale = reg.FindNode("Api_Time_GetScale");
    ENJIN_ASSERT_TRUE(getScale != nullptr);
    ENJIN_EXPECT_TRUE(getScale->IsPure());
    ENJIN_EXPECT_TRUE(getScale->evaluate != nullptr);

    ExecutionContext ctx;
    ctx.scriptEngine = &se;
    Scripting::SetTimeScale(1.0f);
    ECS::VariableValue v = getScale->evaluate(ctx, {});
    ENJIN_ASSERT_TRUE(std::holds_alternative<f32>(v));
    ENJIN_EXPECT_FLOAT_EQ(std::get<f32>(v), 1.0f);

    // A generated FLOW node with an argument: Time_SetScale(0.5) then read
    // it back through the generated getter - proves arg marshaling.
    const NodeDefinition* setScale = reg.FindNode("Api_Time_SetScale");
    ENJIN_ASSERT_TRUE(setScale != nullptr);
    ENJIN_EXPECT_TRUE(!setScale->IsPure());
    std::vector<ECS::VariableValue> outs;
    setScale->execute(ctx, { 0.5f }, outs);
    v = getScale->evaluate(ctx, {});
    ENJIN_EXPECT_FLOAT_EQ(std::get<f32>(v), 0.5f);
    Scripting::SetTimeScale(1.0f);   // restore for other tests
