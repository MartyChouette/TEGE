...
static const std::unordered_map<std::string, ComponentHelp>& Registry() {
    static const std::unordered_map<std::string, ComponentHelp> reg = [] {
        ...
        r["parallaxLayer"] = { "Turns this sprite into a parallax layer that scrolls at a fraction of the camera's movement.", "Set Parallax Factor per axis: 0 locks it to the camera (far backdrop), 1 moves it fully with the world (foreground), values between give depth. It rides the normal sprite pipeline, so stack several sprites at different factors and sorting layers.", nullptr, { { RelationKind::PairsWith, "Sprite 2D", nullptr, nullptr } } };
