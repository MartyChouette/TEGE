...
static PFN_vkCmdTraceRaysKHR s_vkCmdTraceRaysAO = nullptr;

RTAmbientOcclusion::RTAmbientOcclusion(VulkanContext* context) : m_Context(context) {}
RTAmbientOcclusion::~RTAmbientOcclusion() { Shutdown(); }

bool RTAmbientOcclusion::Initialize(u32 width, u32 height, VkDescriptorSetLayout rtDescLayout) {
    m_Width = width;
    m_Height = height;

    s_vkCmdTraceRaysAO = (PFN_vkCmdTraceRaysKHR)vkGetDeviceProcAddr(m_Context->GetDevice(), "vkCmdTraceRaysKHR");
    if (!s_vkCmdTraceRaysAO) return false;
    ...
    if (vkCreatePipelineLayout(m_Context->GetDevice(), &layoutInfo, nullptr, &m_PipelineLayout) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTAmbientOcclusion: Failed to create pipeline layout");
        return false;
    }
    ...
    std::vector<RTPipeline::ShaderStage> stages = {
        { VK_SHADER_STAGE_RAYGEN_BIT_KHR, RT_AO_RGEN_SPV, sizeof(RT_AO_RGEN_SPV) },
        { VK_SHADER_STAGE_MISS_BIT_KHR, RT_AO_RMISS_SPV, sizeof(RT_AO_RMISS_SPV) },
        { VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, RT_AO_RCHIT_SPV, sizeof(RT_AO_RCHIT_SPV) }
    };
    ...
    if (!m_Pipeline->Create(stages, groups, rtDescLayout, m_PipelineLayout, 1)) {
        ENJIN_LOG_ERROR(Renderer, "Failed to create RT AO pipeline");
        return false;
    }

    m_Initialized = true;
    ENJIN_LOG_INFO(Renderer, "RT AO initialized (%ux%u)", width, height);
    return true;
}

void RTAmbientOcclusion::Resize(u32 width, u32 height) {
...
void RTAmbientOcclusion::Dispatch(VkCommandBuffer cmd, VkDescriptorSet rtDescSet,
                                   const Math::Matrix4& invViewProj, const Math::Vector3& cameraPos,
                                   u32 frameCount) {
    if (!m_Initialized || !m_Config.enabled || !s_vkCmdTraceRaysAO) return;
    ...
    s_vkCmdTraceRaysAO(cmd, &sbt.raygen, &sbt.miss, &sbt.hit, &sbt.callable,
                        m_Width, m_Height, 1);
}

void RTAmbientOcclusion::CreateOutputImage() {
    ...
    if (vkCreateImage(m_Context->GetDevice(), &imgInfo, nullptr, &m_OutputImage) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTAmbientOcclusion: Failed to create output image");
        return;
    }
    ...
    if (vkAllocateMemory(m_Context->GetDevice(), &allocInfo, nullptr, &m_OutputMemory) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTAmbientOcclusion: Failed to allocate image memory");
        return;
    }
    ...
    if (vkCreateImageView(m_Context->GetDevice(), &viewInfo, nullptr, &m_OutputView) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTAmbientOcclusion: Failed to create image view");
        return;
    }
}

void RTAmbientOcclusion::DestroyOutputImage() {
...
void RTAmbientOcclusion::Shutdown() {
