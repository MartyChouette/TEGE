...
static PFN_vkCmdTraceRaysKHR s_vkCmdTraceRaysRefl = nullptr;
...
RTReflections::RTReflections(VulkanContext* context) : m_Context(context) {}
RTReflections::~RTReflections() { Shutdown(); }

bool RTReflections::Initialize(u32 width, u32 height, VkDescriptorSetLayout rtDescLayout) {
    m_Width = width;
    m_Height = height;

    s_vkCmdTraceRaysRefl = (PFN_vkCmdTraceRaysKHR)vkGetDeviceProcAddr(m_Context->GetDevice(), "vkCmdTraceRaysKHR");
    if (!s_vkCmdTraceRaysRefl) return false;
    ...
    if (vkCreatePipelineLayout(m_Context->GetDevice(), &layoutInfo, nullptr, &m_PipelineLayout) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTReflections: Failed to create pipeline layout");
        return false;
    }
    ...
    std::vector<RTPipeline::ShaderStage> stages = {
        { VK_SHADER_STAGE_RAYGEN_BIT_KHR, RT_REFLECT_RGEN_SPV, sizeof(RT_REFLECT_RGEN_SPV) },
        { VK_SHADER_STAGE_MISS_BIT_KHR, RT_REFLECT_RMISS_SPV, sizeof(RT_REFLECT_RMISS_SPV) },
        { VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, RT_REFLECT_RCHIT_SPV, sizeof(RT_REFLECT_RCHIT_SPV) }
    };
    ...
    if (!m_Pipeline->Create(stages, groups, rtDescLayout, m_PipelineLayout, 1)) {
        ENJIN_LOG_ERROR(Renderer, "Failed to create RT reflection pipeline");
        return false;
    }

    m_Initialized = true;
    ENJIN_LOG_INFO(Renderer, "RT Reflections initialized (%ux%u)", width, height);
    return true;
}

void RTReflections::Resize(u32 width, u32 height) {
...
void RTReflections::Dispatch(VkCommandBuffer cmd, VkDescriptorSet rtDescSet,
                              const Math::Matrix4& invViewProj, const Math::Vector3& cameraPos,
                              u32 frameCount) {
    if (!m_Initialized || !m_Config.enabled || !s_vkCmdTraceRaysRefl) return;
    ...
    s_vkCmdTraceRaysRefl(cmd, &sbt.raygen, &sbt.miss, &sbt.hit, &sbt.callable,
                          m_Width, m_Height, 1);
}

void RTReflections::CreateOutputImage() {
    ...
    if (vkCreateImage(m_Context->GetDevice(), &imgInfo, nullptr, &m_OutputImage) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTReflections: Failed to create output image");
        return;
    }
    ...
    if (vkAllocateMemory(m_Context->GetDevice(), &allocInfo, nullptr, &m_OutputMemory) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTReflections: Failed to allocate image memory");
        return;
    }
    ...
    if (vkCreateImageView(m_Context->GetDevice(), &viewInfo, nullptr, &m_OutputView) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTReflections: Failed to create image view");
        return;
    }
}

void RTReflections::DestroyOutputImage() {
