...
static PFN_vkCmdTraceRaysKHR s_vkCmdTraceRaysCaustic = nullptr;
...
RTCaustics::RTCaustics(VulkanContext* context) : m_Context(context) {}
RTCaustics::~RTCaustics() { Shutdown(); }

bool RTCaustics::Initialize(u32 width, u32 height, VkDescriptorSetLayout rtDescLayout) {
    m_Width = width;
    m_Height = height;

    s_vkCmdTraceRaysCaustic = (PFN_vkCmdTraceRaysKHR)vkGetDeviceProcAddr(m_Context->GetDevice(), "vkCmdTraceRaysKHR");
    if (!s_vkCmdTraceRaysCaustic) return false;
    ...
    if (vkCreatePipelineLayout(m_Context->GetDevice(), &layoutInfo, nullptr, &m_PipelineLayout) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTCaustics: Failed to create pipeline layout");
        return false;
    }
    ...
    std::vector<RTPipeline::ShaderStage> stages = {
        { VK_SHADER_STAGE_RAYGEN_BIT_KHR, RT_CAUSTICS_RGEN_SPV, sizeof(RT_CAUSTICS_RGEN_SPV) },
        { VK_SHADER_STAGE_MISS_BIT_KHR, RT_CAUSTICS_RMISS_SPV, sizeof(RT_CAUSTICS_RMISS_SPV) },
        { VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, RT_CAUSTICS_RCHIT_SPV, sizeof(RT_CAUSTICS_RCHIT_SPV) }
    };
    ...
    if (!m_Pipeline->Create(stages, groups, rtDescLayout, m_PipelineLayout, 2)) {
        ENJIN_LOG_ERROR(Renderer, "Failed to create RT caustics pipeline");
        return false;
    }

    m_Initialized = true;
    ENJIN_LOG_INFO(Renderer, "RT Caustics initialized (%ux%u)", width, height);
    return true;
}

void RTCaustics::Resize(u32 width, u32 height) {
...
void RTCaustics::Dispatch(VkCommandBuffer cmd, VkDescriptorSet rtDescSet,
                            const Math::Matrix4& invViewProj, const Math::Vector3& lightDir,
                            u32 frameCount) {
    if (!m_Initialized || !m_Config.enabled || !s_vkCmdTraceRaysCaustic) return;
    ...
    s_vkCmdTraceRaysCaustic(cmd, &sbt.raygen, &sbt.miss, &sbt.hit, &sbt.callable,
                              m_Width, m_Height, 1);
}

void RTCaustics::CreateOutputImage() {
    ...
    if (vkCreateImage(m_Context->GetDevice(), &imgInfo, nullptr, &m_OutputImage) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTCaustics: Failed to create output image");
        return;
    }
    ...
    if (vkAllocateMemory(m_Context->GetDevice(), &allocInfo, nullptr, &m_OutputMemory) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTCaustics: Failed to allocate image memory");
        return;
    }
    ...
    if (vkCreateImageView(m_Context->GetDevice(), &viewInfo, nullptr, &m_OutputView) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTCaustics: Failed to create image view");
        return;
    }
}

void RTCaustics::DestroyOutputImage() {
