...
bool RTTranslucency::Initialize(u32 width, u32 height, VkDescriptorSetLayout rtDescLayout) {
    ...
    if (vkCreatePipelineLayout(m_Context->GetDevice(), &layoutInfo, nullptr, &m_PipelineLayout) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTTranslucency: Failed to create pipeline layout");
        return false;
    }
...
void RTTranslucency::CreateOutputImage() {
    ...
    if (vkCreateImage(m_Context->GetDevice(), &imgInfo, nullptr, &m_OutputImage) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTTranslucency: Failed to create output image");
        return;
    }
    ...
    if (vkAllocateMemory(m_Context->GetDevice(), &allocInfo, nullptr, &m_OutputMemory) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTTranslucency: Failed to allocate image memory");
        return;
    }
    ...
    if (vkCreateImageView(m_Context->GetDevice(), &viewInfo, nullptr, &m_OutputView) != VK_SUCCESS) {
        ENJIN_LOG_ERROR(Renderer, "RTTranslucency: Failed to create image view");
        return;
    }
}
