...
bool UDPTransport::Bind(u16 port) {
    ...
    if (!m_WsaInitialized) {
        if (s_WsaRefCount == 0) {
            ...
            if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
                ENJIN_LOG_ERROR(Network, "UDPTransport: WSAStartup failed");
                return false;
            }
        }
        s_WsaRefCount++;
    ...
    if (sock == INVALID_SOCK) {
        ENJIN_LOG_ERROR(Network, "UDPTransport: Failed to create socket");
        return false;
    }
    ...
    if (ioctlsocket(sock, FIONBIO, &mode) != 0) {
        ENJIN_LOG_ERROR(Network, "UDPTransport: Failed to set non-blocking");
        closesocket(sock);
        return false;
    }
#else
    ...
    if (bind(sock, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) != 0) {
        ENJIN_LOG_ERROR(Network, "UDPTransport: Failed to bind to port %u", port);
#ifdef _WIN32
        closesocket(sock);
#else
        close(sock);
    ...
    ENJIN_LOG_INFO(Network, "UDPTransport: Bound to port %u", m_BoundPort);
    return true;
}

bool UDPTransport::SendTo(const NetworkAddress& addr, const u8* data, u32 size) {
...
i32 UDPTransport::ReceiveFrom(NetworkAddress& sender, u8* buffer, u32 bufferSize) {
    if (!m_Open) return -1;
...
void UDPTransport::Close() {
    ...
    ENJIN_LOG_INFO(Network, "UDPTransport: Socket closed");
}
