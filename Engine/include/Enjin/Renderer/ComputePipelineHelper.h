...
struct ENJIN_API ComputePipelineSetup {
    ...
    bool Create(VulkanContext* context,
        ...
        if (!loaded) {
            // Shipped Player builds have no shader tree on disk — fall back to
            // the embedded registry (generated by _gen_compute.py).
            if (const EmbeddedComputeShader* embedded = FindEmbeddedComputeShader(shaderName)) {
                loaded = shader.LoadFromSPIRV(embedded->data, embedded->size) &&
                         shader.GetModule() != VK_NULL_HANDLE;
                if (loaded) {
                    ENJIN_LOG_INFO(Renderer, "ComputePipelineHelper: '%s' loaded from embedded SPIR-V", shaderName);
                }
            }
        }
        if (!loaded) {
