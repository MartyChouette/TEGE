...
struct ENJIN_API MeshComponent : public IComponent {
    ...
    struct SourceRef {
        ...
        // Which LOD level of that mesh. 0 is the mesh as the file contains it; 1..4 are
        // GENERATED by MeshSimplifier and exist nowhere in the source file, so they can
        // only ever come from the baked cache -- re-importing cannot produce them. The
        // level has to be part of the reference (and of the cache key) or a simplified
        // level adopts itself over LOD 0's cache entry, which silently replaces the
        // full-detail mesh with a coarse one everywhere that reference is used.
        i32   lodLevel = 0;
