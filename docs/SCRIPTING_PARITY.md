...
- **Unbound components/systems.** A capability is only reachable from script if a binding exists. Components with no accessor functions yet. New engine features default to C++-only until bound. *Progress 2026-08-13:* the previously-unbound gameplay/visual components are now bound (`ScriptBindings_GameplayComponents.cpp`) — batch 1: LookAtTarget, DamageResistance, Ragdoll, Pushable, TemperatureZone, ReflectionProbe, Billboard; batch 2: Possessable, SavePoint, Footstep, ReverbZone, Lens, and the physics joint components (Spring/Slider/Fixed/BallSocket). Data/editor-internal components (Mesh, Skeleton, StableId, Script, Save*, hierarchy, network internals) and big-data/procedural ones (Terrain, vegetation volumes) are intentionally not script-bound. The script surface is now close to full gameplay parity with C++.
