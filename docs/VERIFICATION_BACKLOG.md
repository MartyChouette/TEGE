...
| **Physics (Jolt 3D + Box2D 2D)** | **GOOD** | `TestPhysicsSimulation` steps both backends and asserts behavior: box falls and rests on a floor (Jolt + Box2D), 3D raycast hits, gravity round-trips, bilateral collision filtering suppresses contacts (incompatible masks tunnel through), collider size is world-space (ignores entity scale), `CharacterVirtual` grounds with normal up, and 2D sensor enter events fire. The former nice-to-haves are covered too: `PhysicsSim2D.RaycastSkipsSensors` (sensor between ray and wall — the wall is reported), `PhysicsSim2D.DistanceJointMaintainsSeparation` (rigid distance joint hangs a weight at joint length under gravity), and `GameplayHazards.HazardOverlapDamagesPlayerWithoutSensorEvents` (the `CheckHazardOverlaps` kinematic-kinematic workaround damages on overlap, honors `damageOnce`, ignores distant players). |
