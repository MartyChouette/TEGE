...
- **DOTS-competitive batch type**: Tier-1 SoA `Swarm` PROTOTYPED (`Core/include/Enjin/Sim/Swarm.h`, TestSwarm ~300M agent-updates/sec single-thread; editor `SwarmComponent`+`SwarmSystem` visualizes via instanced cube proxies). NEXT: (a) fork-join split across cores, (b) a real GPU-instanced draw (position buffer → one draw, replaces proxy entities), (c) **Tier-2 GPU-compute agents = the actual DOTS-beater, esp. on web where DOTS can't run — BLOCKED on WebGPU backend gaining compute pipelines** (only Vulkan has them today). Honest verdict: TEGE ECS does NOT match DOTS on raw million-entity throughput and doesn't need to (200-dog test was GPU/draw-bound); the Swarm type is the escape hatch for the cases that do need scale.
