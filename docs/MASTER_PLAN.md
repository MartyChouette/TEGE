...
- Node editor: work on the VisualScript node graph editor (blueprint-style visual scripting, `Engine/*/VisualScript/`, ~262 nodes). Scope TBD with Marty (usability, missing node coverage, graph UX).
- Node + AngelScript parity with C++ features: make the three scripting surfaces reach the same capability. Audit what C++ can do that AngelScript bindings (~1009) and VisualScript nodes cannot, and close the gaps so a game authored in script/nodes is not second-class. (Pairs with the "~35 unbound components" item under Scripting and serialization surface.)
