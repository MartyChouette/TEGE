...
| Multiplayer / networking | **Absent (browser side)** | A browser build still has no network transport: `TransportFactory` returns `nullptr` on web. Browser networking is PARKED as of 2026-09-20 -- `WebSocketTransportWeb` is written and did compile and link for web via `emscripten_websocket`, but a browser completing a game join against a live host was never demonstrated, and it is compiled out behind `ENJIN_WEB_NETWORKING` rather than shipped on the strength of compiling. What DID land is the other half: a desktop host can serve WebSocket (`EnjinPlayer --server --websocket`), verified against an independent RFC 6455 client and against real headless Chrome. So there is something for a browser to connect to whenever the browser end is picked back up. Desktop multiplayer is UDP on a LAN or a port-forwarded direct IP. Do not design a web game around multiplayer. |
