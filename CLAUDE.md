...
- **Component Search Bar** — ~~DONE~~ Searchable Add Component popup with case-insensitive substring filter across 14 categories (Rendering, Physics, Audio, Gameplay, Controllers, Joints, AI, Visual, Effects, 2D Graphics, Scripting, Puzzle, etc.). Collapsible category headers, 5-item recently-used pinning with disk persistence, keyboard navigation (arrow keys + Enter). Future work: upgrade substring to fuzzy matching (Levenshtein distance).
