...
`TemplateCreator` saves the current scene as a reusable startup template. `SaveTemplate()` / `LoadTemplate()` / `ScanTemplates()` / `DeleteTemplate()`. Editor UI at View > Tools > Template Creator with metadata editing (name, description, category), save/load/delete. Custom templates stored in `templates/` directory. Files: `TemplateCreator.h/cpp`

### Scripting

- **AngelScript** via `TegeBehavior` base class with hot-reload
- ~430 bound functions across math, entity, scene, input, physics, physics 2D, audio, components (incl. full animator: play/stop/pause/resume/isPlaying/getAnimation/speed), sprite 2D (texture/color/flip/sort/anim), coroutines, events, tweening, noise, rendering, post-processing, dialogue, save/load, weather, particles, quests, cinematics, object pool, destructibles, UI canvas (including focus management), localization, prefabs, networking, AI/behavior trees (15 AI controller + 13 BT blackboard + 2 navmesh), accessibility (8 subtitle + 5 announcer + 4 colorblind + 3 general), procedural generation (~15 bindings for all 9 algorithms), camera presets, Newgrounds, audio event graph (4 bindings: TriggerEvent/SetParameter/GetParameter/StopAll), plugins (4 bindings: IsLoaded/GetVersion/Load/Unload), Flash API shim (~40 bindings: DisplayObject, MovieClip, Stage, Mouse, Keyboard, TextField, Math, Sound, Timer)
- See `docs/SCRIPTING_API.md` for the complete API reference
- **Visual scripting** (Blueprint-style) with 134+ built-in nodes (including 25 Gameplay nodes: save/load/checkpoint/meta, weather, quests, cinematics, particles, destructibles, prefabs, UI focus, localization; 5 Physics 2D nodes; 6 Networking nodes; 4 AI + 4 BT + 1 Navmesh; 2 StateMachine; 3 Accessibility; 4 Tween; 3 Dialogue; 2 Animator; 4 Input; 2 Scene; 6 Noise; 6 Streaming; 9 Procedural; 3 AudioGraph; 3 Plugin; 6 Water; 2 HUD), debugger with breakpoints/step-through, execution timeline profiler
...
The engine has 145+ completed features across these categories. See `docs/USER_MANUAL.md` for component details.

- **Rendering:** Vulkan with Blinn-Phong, PBR materials, normal/parallax mapping (4 POM modes), 4-cascade CSM shadows, post-processing (bloom, vignette, FXAA, film grain, color grading, full-screen stipple/dither, depth of field, tilt-shift), retro effects, wireframe, deferred framework, GPU frustum culling, per-scene render settings, ray tracing pipeline (RT shadows, reflections, AO, GI, path tracing, SVGF denoiser + optional OIDN via `ENJIN_RAYTRACING_OIDN` — awaiting compiled SPIR-V shaders), SH light probes (L2, grid generation, baking, irradiance queries), SDF scene (6 primitives, 6 boolean ops, CPU evaluation, GPU buffer packing), OIT (weighted blended, accumulation + revealage textures), camera presets (9 built-in), dithered gradient rendering (per-material flat-shaded banding, 2-8 bands, 6 dither patterns)
- **ECS & Editor:** 70+ component types, ImGui editor with hierarchy/inspector/viewport, transform gizmos, multi-select, undo/redo, component search with fuzzy matching, 43 startup templates + template creator (save scene as reusable template), entity visibility toggle, bug reporting & feedback system (auto-diagnostics, JSON persistence, remote submission), vector drawing editor (7 shapes, layers, SVG export), splash screen with animated mantra, source-app import presets (10 DCC tools with auto-detection), collaborative editing UI (OT protocol, peer cursors, conflict resolution, lock enforcement), symbol library (reusable graphic symbols as prefabs, nested editing, category browser, update propagation, Flash timeline sync)
...
- **Scripting:** AngelScript (~610 bindings incl. 22 gameplay component types, AI/BT/navmesh, accessibility, procedural gen, audio graph, plugins, Flash API shim ~40 bindings), AS2/AS3→AngelScript transpiler, visual scripting (126+ nodes, debugger), state machines with script callbacks, coroutines, event system, DataAsset system (schemas + instances, JSON I/O, AS + VS bindings), documentation generator, plugin DLL repositories
...
- **Editor Tools:** ~~Accent color theming~~ (done), project hub, ~~template rebuild~~ (done — 43 templates, all polished with ~50% more content), ~~extended model formats (PLY/VOX)~~ (done), drag-and-drop improvements, ~~micro-interactions~~ (done — spring easing, hover transitions), ~~template creator~~ (done — save scene as template, View > Tools), ~~source-app import presets~~ (done — 10 DCC tools with auto-detection), ~~font library~~ (done — 42 fonts, 8 categories), ~~2D/3D asset library~~ (done — 16 3D + 15 2D CC0 packs)
- **Runtime Systems:** ~~Improved physics (2D, CCD)~~ (done — PhysicsWorld2D), networking, ~~destructible environments~~ (done — DestructibleSystem), ~~fluid simulation~~ (done — Stable Fluids solver, FluidTerrainCoupling), ~~SVG import~~ (done), ~~dialogue assets + localization~~ (done — .enjdlg files, LocalizationManager)
- **Rendering & Performance:** ~~Sprite batching~~ (done), ~~pipeline optimization~~ (done), ~~soft shadows~~ (done), ~~ray tracing pipeline~~ (done — RT shadows/reflections/AO/GI, path tracing, SVGF + OIDN denoisers, awaiting compiled SPIR-V), ~~full-screen stipple/dither~~ (done — 8 combinable patterns via bitmask, 3 color modes, post-process chain), ~~camera presets~~ (done — 9 presets), ~~depth of field + tilt-shift~~ (done — infrastructure, awaiting SPIR-V), ~~SH light probes~~ (done — L2, grid, baking), ~~SDF scene~~ (done — 6 primitives, boolean ops), ~~OIT~~ (done — weighted blended), ~~POM advanced modes~~ (done — Steep/Occlusion/Relief), ~~Jolt + Box2D physics integration~~ (done — all 5 phases: abstraction, Jolt backend, Box2D backend, production defaults, SimplePhysics compile-guarded), ~~dithered gradient rendering~~ (done — per-material flat-shaded banding, 2-8 bands, 6 patterns), ~~binary distribution~~ (done — CMake install + CPack, package scripts)
- **Procedural Generation:** ~~All algorithms~~ (done — 9+ algorithms + editor panel, fractal terrain with erosion, advanced 3D L-system), ~~custom flora assets~~ (done)
- **Scripting:** ~~Plugin DLL repositories~~ (done), ~~documentation generator~~ (done), ~~ScriptableObject/DataAsset system~~ (done)
- **Graph Systems:** ~~Shader Graph~~ (done — 54 node types, full GLSL codegen with topological sort, .enjshader save/load), ~~Audio Event Graph~~ (done — runtime execution via SimpleAudio, trigger/parameter/delay, .enjaudiopkg, 4 AS + 3 VS bindings), ~~Particle Graph~~ (done — compiler to ParticleEmitterComponent, .enjparticle save/load)
...
- **Flash Game Revival:** ~~Vector drawing editor~~ (done), ~~Newgrounds.io API~~ (done), ~~HTML5 export~~ (done), ~~SWF import~~ (done — SWFConverter), ~~AS2/AS3 transpiler~~ (done — AS3Transpiler + FlashAPIShim), ~~symbol library~~ (done — reusable symbols as prefabs, nested editing, Flash timeline sync), ~~Newgrounds game page~~ (done — themed HTML5 template with medals/scoreboard/embed), Flash-style authoring
