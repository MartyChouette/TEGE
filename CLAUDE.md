...
- **Render pass formats:** Swapchain uses `VK_FORMAT_B8G8R8A8_SRGB`, offscreen `RenderTarget`s use `VK_FORMAT_B8G8R8A8_UNORM`. Main pipeline created for SRGB — effect pipelines recreated for UNORM via `RecreateEffectPipelinesForRenderPass()`. **Known issue:** main `m_Pipeline` is never recreated for UNORM, causing Vulkan spec violation when used in offscreen targets (NVIDIA tolerates for 3D, may crash for 2D).
