...
- **Rendering:** Vulkan with Blinn-Phong, PBR materials, normal/parallax mapping (4 POM modes), 4-cascade CSM shadows, post-processing (bloom, vignette, FXAA, film grain, color grading, full-screen stipple/dither, depth of field, tilt-shift), retro effects, wireframe, deferred framework, GPU frustum culling, per-scene render settings, ray tracing pipeline (RT shadows, reflections, AO, GI, path tracing, SVGF denoiser + optional OIDN via `ENJIN_RAYTRACING_OIDN` — all 19 SPIR-V shaders compiled and embedded), SH light probes (L2, grid generation, baking, wired to renderer via LightingUBO, editor baking tool + viewport visualization), SDF scene (6 primitives, 6 boolean ops, CPU evaluation, GPU buffer packing), OIT (weighted blended, composite pipeline with fullscreen shader), camera presets (9 built-in), dithered gradient rendering (per-material flat-shaded banding, 2-8 bands, 6 dither patterns)
...
- **Scripting:** AngelScript (~615 bindings incl. 22 gameplay component types, AI/BT/navmesh, accessibility, procedural gen, audio graph, plugins, Flash API shim ~40 bindings, Flash SharedObject persistence 5 bindings), AS2/AS3→AngelScript transpiler, visual scripting (126+ nodes, debugger), state machines with script callbacks, coroutines, event system, DataAsset system (schemas + instances, JSON I/O, AS + VS bindings), documentation generator, plugin DLL repositories
...
- **Networking:** LAN multiplayer (host-authoritative UDP, client-side prediction, 20Hz state sync, interpolation buffer, entity ownership, RPC system, lobby, reliable delivery, HMAC-SHA256 packet authentication with replay protection, editor Network Panel), cross-platform HTTP client (WinHTTP + libcurl)
...
- **Editor Tools:** ~~Accent color theming~~ (done), project hub, ~~template rebuild~~ (done — 44 templates, all polished with ~50% more content), ~~extended model formats (PLY/VOX)~~ (done), drag-and-drop improvements, ~~micro-interactions~~ (done — spring easing, hover transitions), ~~template creator~~ (done — save scene as template, View > Tools), ~~source-app import presets~~ (done — 10 DCC tools with auto-detection), ~~font library~~ (done — 42 fonts, 8 categories), ~~2D/3D asset library~~ (done — 16 3D + 15 2D CC0 packs), ~~gamepad editor~~ (done — 4 radial dials, DPad hierarchy, stick camera)
