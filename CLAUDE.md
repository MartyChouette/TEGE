...
The engine has 130+ completed features across these categories. See `docs/USER_MANUAL.md` for component details.
...
- **Build & Export:** Asset pack pipeline (.enjpak), standalone player app, splitscreen (2P/4P), HTML5 export (canvas, preloader, responsive scaling, Newgrounds-compatible embed), binary distribution (CMake install + CPack, Windows ZIP, package scripts), Linux AppImage builder, SWF import/conversion (shapes→sprites, MovieClips→entity hierarchy)
...
- **Flash Game Revival:** ~~Vector drawing editor~~ (done), ~~Newgrounds.io API~~ (done), ~~HTML5 export~~ (done), ~~SWF import~~ (done — SWFConverter), ~~AS2/AS3 transpiler~~ (done — AS3Transpiler + FlashAPIShim), Flash-style authoring
