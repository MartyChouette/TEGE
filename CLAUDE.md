...
- **`MaterialGPU` = 128 bytes** — struct alignment matters for SSBO offsets (guarded by `static_assert` in TestMaterial). Was 80 (pre-SSS), then 112 (SSS + bindless indices), now 128 (scrolling-reflection row: scrollReflTexIdx + scroll speed + strength). Keep this in lockstep across THREE places: the C++ `MaterialGPU` struct, the shader `MaterialEntry` struct (triangle.frag), and the `static_assert` in TestMaterial — then regen ShaderData.h and verify a golden capture is byte-identical (a mismatch corrupts ALL materials)
