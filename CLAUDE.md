...
- **Entity Visibility Toggle** — Add a `visible` bool to entities (or a `VisibilityComponent` / flag on `TransformComponent`) that allows game objects to be hidden/shown at runtime without destroying them or moving them offscreen. RenderSystem should skip drawing invisible entities. Inspector checkbox, hierarchy eye icon, and script binding (`Entity_SetVisible(uint64, bool)`, `Entity_IsVisible(uint64)`). This is essential for gameplay (hiding/showing objects, pooling, LOD, etc.) and avoids the current workaround of moving entities to y=-100.
...
- **Tweening System** — General-purpose tween library accessible from both C++ and AngelScript. Tween any numeric property (position, rotation, scale, color, opacity, float fields) over time with standard easing functions (Linear, EaseIn/Out/InOut Quad, Cubic, Quart, Quint, Sine, Expo, Circ, Back, Elastic, Bounce). API: `Tween::To(target, duration, easing)`, `Tween::From(start, duration, easing)`, chainable `.OnComplete(callback)`, `.Delay(seconds)`, `.Loop(count)`, `.PingPong()`. Usable everywhere: UI animations (button hover, panel slide-in, fade transitions), gameplay (health bar lerp, damage flash, pickup bob), camera (smooth zoom, shake decay), effects (color pulse, scale pop). Should integrate with the existing Timeline/Sequencer for editor-authored tweens, but also work standalone for script-driven animation. Script bindings: `Tween_Position(entity, target, duration, easing)`, `Tween_Scale(entity, target, duration, easing)`, `Tween_Color(entity, target, duration, easing)`, `Tween_Float(startVal, endVal, duration, easing, callback)`. Managed by a `TweenSystem` that ticks all active tweens each frame, auto-cleans completed ones. Replaces hand-written lerp code scattered across systems.
