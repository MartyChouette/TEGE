...
- **Rendering & Performance:** ~~Sprite batching~~ (done), ~~pipeline optimization~~ (done), ~~soft shadows~~ (done), ~~ray tracing pipeline~~ (done — RT shadows/reflections/AO/GI, path tracing, SVGF denoiser, awaiting compiled SPIR-V), ~~full-screen stipple/dither~~ (done — 8 patterns, 3 color modes, post-process chain), Jolt + Box2D physics integration (Phases 1-3 done — IPhysicsBackend abstraction + JoltBackend + Box2DBackend; Phases 4-5 pending — migration, SimplePhysics retirement)
