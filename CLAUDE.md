...
`IPhysicsBackend`/`IPhysicsBackend2D` interfaces. Backends: Jolt v5.2.0 (3D), Box2D v3.0.0 (2D), SimplePhysics (legacy, compile-guarded). `PhysicsBackendFactory` creates via type enum (`Auto`/`Jolt`/`Box2D`/`Simple`). CMake: `ENJIN_PHYSICS_JOLT` (ON), `ENJIN_PHYSICS_BOX2D` (ON), `ENJIN_PHYSICS_SIMPLE` (ON).
