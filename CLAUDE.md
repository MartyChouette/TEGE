...
- **Render pass formats:** Swapchain uses `VK_FORMAT_B8G8R8A8_SRGB`, offscreen `RenderTarget`s use `VK_FORMAT_B8G8R8A8_UNORM`. Main pipeline created for SRGB — `m_OffscreenPipeline` (+ line/outline variants) created for UNORM in `RecreateEffectPipelinesForRenderPass()`. Offscreen render targets use `VK_SAMPLE_COUNT_1_BIT` (no MSAA).
