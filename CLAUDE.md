...
- **Rendering & Performance:** ~~Sprite batching~~ (done), ~~pipeline optimization~~ (done), ~~soft shadows~~ (done), ~~ray tracing pipeline~~ (done — RT shadows/reflections/AO/GI, path tracing, SVGF denoiser, awaiting compiled SPIR-V), Jolt + Box2D physics integration (Phases 1-3 done — IPhysicsBackend abstraction + JoltBackend + Box2DBackend; Phases 4-5 pending — migration, SimplePhysics retirement)
