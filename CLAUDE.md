...
- **THE EDITOR HAS TWO `InputActionMap`s** — `EditorLayer::m_InputMap` (what `ControllerSystem` and `GameMenuSystem` actually read) and `PlayMode::m_InputMap` (what script bindings read). Project settings are applied to BOTH at play start and the `ActionTriggerSystem` is re-pointed at EditorLayer's, or triggers read stale bindings. Collapsing these two is Phase 4 work
