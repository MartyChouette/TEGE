...
- **WGSL is compiled by the BROWSER, so a green web build proves nothing about it.** Run `cd tools && npm install && node check_wgsl.mjs` after ANY edit to `WebShaderData.h` — it compiles all 13 shaders through Dawn, the same compiler Chrome uses, and reports errors with line numbers. Without it a syntax or type error reaches a player as a black canvas. CI runs it in the web job. (Headless Chrome on this machine exposes `navigator.gpu` but returns no adapter, so a browser cannot be used for this)
